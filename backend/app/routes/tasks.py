...
async def create_task(data: TaskCreate):
    db = get_db()
    payload = build_task_payload(data.model_dump())
...
async def get_task(task_id: str):
    db = get_db()
    task = await db.tasks.find_one({"_id": parse_object_id(task_id)})
    if not task:
...
async def update_task(task_id: str, data: TaskUpdate):
    ...
    if "start_time" in update_data or "end_time" in update_data:
        existing = await db.tasks.find_one({"_id": task_object_id})
        if not existing:
            raise HTTPException(status_code=404, detail="Task not found")
    ...
    update_data = build_task_payload(update_data, is_update=True)
